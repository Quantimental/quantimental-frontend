...
import { StockNewsHeadlines } from './stock-news-headlines'
import { ArticleReportModal } from './article-report-modal'
import type { Stock } from './dashboard'
import { getSignalFreshness, generateSimpleExplanation } from '@/lib/market-engine'
...
export function StockCard({ stock, onRemove }: StockCardProps) {
  ...
  const [signalFreshness, setSignalFreshness] = useState<'new' | 'fresh' | 'recent' | 'stale'>('recent')
  
  // Calculate freshness on client-side only to avoid hydration mismatch
  useEffect(() => {
    if (stock.signalAge) {
      setSignalFreshness(getSignalFreshness(stock.signalAge))
    }
  }, [stock.signalAge])
  ...
  // Generate simple explanation for tagline
  const tagline = stock.tagline || generateSimpleExplanation(
    stock.technicalRating || 50,
    stock.sentimentRating || 50,
    stock.technicalDetails?.rsi || 50,
    stock.price || 0,
    stock.technicalDetails?.ma50 || stock.price || 0,
    stock.sentimentDetails?.score || 0,
    stock.sentimentDetails?.mentions || 0,
    stock.sentimentDetails?.velocity || 'steady',
    stock.signal
  )
      ...
      <Card className="p-6 hover:shadow-lg hover:shadow-accent/30 transition-smooth animate-fade-in-up">
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            <div className="text-3xl font-bold font-mono transition-smooth">
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent side="bottom">
                      <div className="max-w-xs text-sm space-y-1">
                        <p><strong>RSI (Relative Strength Index):</strong> Measures momentum. Above 70 = overbought (may pull back), below 30 = oversold (may bounce), 30-70 = neutral.</p>
                        <p><strong>MA50 ${(stock.technicalDetails?.ma50 || 0).toFixed(2)}:</strong> 50-day moving average. Price above = uptrend bias, below = downtrend bias.</p>
                      </div>
                    </TooltipContent>
                  </Tooltip>
                </div>
                <span className="font-semibold font-mono">{(stock.technicalRating || 0)}/100</span>
              </div>
              <Progress value={stock.technicalRating || 0} className="h-2" />
              <div className="text-xs text-muted-foreground space-y-0.5">
                <div>
                  RSI: {stock.technicalDetails?.rsi || 0}
                  {stock.technicalDetails?.rsi && stock.technicalDetails.rsi > 70 ? ' 🔴' : stock.technicalDetails.rsi < 30 ? ' 🟢' : ''}
                </div>
                <div>
                  MA50: ${(stock.technicalDetails?.ma50 || 0).toFixed(2)}
                  {stock.price && stock.technicalDetails?.ma50 && (stock.price > stock.technicalDetails.ma50 ? ' 📈' : ' 📉')}
                </div>
              </div>
            </div>
              ...
              <div className="flex items-center justify-between text-sm">
                <div className="flex items-center gap-1">
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent side="bottom">
                      <div className="max-w-xs text-sm space-y-1">
                        <p><strong>Sentiment {(stock.sentimentDetails?.score || 0).toFixed(2)}:</strong> {
                          (stock.sentimentDetails?.score || 0) > 0.75 ? 'Extremely bullish - high conviction'
                          : (stock.sentimentDetails?.score || 0) > 0.5 ? 'Positive sentiment'
                          : (stock.sentimentDetails?.score || 0) > 0.25 ? 'Slightly positive'
                          : 'Negative sentiment'
                        }</p>
                        <p><strong>Mentions {(stock.sentimentDetails?.mentions || 0)}:</strong> {
                ...
                <span className="font-semibold font-mono">{(stock.sentimentRating || 0)}/100</span>
              </div>
              <Progress value={stock.sentimentRating || 0} className="h-2" />
              <div className="text-xs text-muted-foreground space-y-0.5">
                <div>Score: {(stock.sentimentDetails?.score || 0).toFixed(2)}</div>
                <div>Mentions: {(stock.sentimentDetails?.mentions || 0).toLocaleString()}</div>
              </div>
            </div>
          </div>
            ...
            <div className="p-3 rounded-lg bg-accent/5 border border-accent/20 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div className="text-muted-foreground">
                  Technical: <span className="font-semibold">{((stock.hybridWeights.technical || 0) * 100).toFixed(0)}%</span>
                </div>
                <div className="text-muted-foreground">
                  Sentiment: <span className="font-semibold">{((stock.hybridWeights.sentiment || 0) * 100).toFixed(0)}%</span>
                </div>
              </div>
            </div>
          )}
          ...
          <div className="space-y-2">
            ...
            <div className="relative h-3 rounded-full bg-gradient-to-r from-destructive via-muted to-success overflow-hidden">
              <div 
                ...
                style={{ 
                  left: `${((stock.technicalRating || 0) + (stock.sentimentRating || 0)) / 2}%`,
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-1 text-xs text-muted-foreground">
                    <div className="flex justify-between">
                      <span>RSI:</span>
                      <span className="font-semibold text-foreground">{stock.technicalDetails?.rsi || 0}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>MA50:</span>
                      <span className="font-semibold text-foreground">${(stock.technicalDetails?.ma50 || 0).toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>MA200:</span>
                      <span className="font-semibold text-foreground">${(stock.technicalDetails?.ma200 || 0).toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between pt-2 border-t border-border/30">
                      <span>Price vs MA50:</span>
                      <span className={stock.price && stock.technicalDetails?.ma50 && stock.price > stock.technicalDetails.ma50 ? 'text-success font-semibold' : 'text-destructive font-semibold'}>
                        {stock.technicalDetails?.ma50 && stock.price ? ((((stock.price - stock.technicalDetails.ma50) / stock.technicalDetails.ma50) * 100)).toFixed(1) : '0.0'}%
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-1 text-xs text-muted-foreground">
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold text-foreground">{((stock.sentimentDetails?.score || 0) * 100).toFixed(0)}%</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Total Mentions:</span>
                      <span className="font-semibold text-foreground">{(stock.sentimentDetails?.mentions || 0).toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Velocity:</span>
                    ...
                    <div className="flex justify-between pt-2 border-t border-border/30">
                      ...
                      <span className="font-semibold text-foreground">
                        {((stock.sentimentRating || 0) - (stock.technicalRating || 0) > 0 ? '+' : '')}{((stock.sentimentRating || 0) - (stock.technicalRating || 0))}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div className="p-3 rounded-lg bg-muted/30 border border-border/30 space-y-2">
                ...
                <div className="space-y-1 text-xs text-muted-foreground">
                  <p>
                    ...
                    {stock.price && stock.technicalDetails?.ma200 && stock.price > stock.technicalDetails.ma200
                      ? 'Uptrend (above MA200)'
                      : 'Downtrend (below MA200)'}
                  </p>
                  <p>
                    <span className="font-semibold">Momentum:</span>{' '}
                    {(stock.technicalDetails?.rsi || 0) > 70
                      ? 'Overbought'
                      : (stock.technicalDetails?.rsi || 0) < 30
                      ? 'Oversold'
                      : 'Neutral'}
                  </p>
                  <p>
